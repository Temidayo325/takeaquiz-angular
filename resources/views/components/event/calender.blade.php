...
<div class="text-purple-1000 px-4 z-20" x-data='{ user: null,
		events: @json($events),
		...
		async shareTicket(ticket)
		{
			...
			let shareData = {
				title: event.name,
				text: event.promotional_copy,
				url: baseUrl + "/ticket/" + ticket.slug,
			}
			try {
				await navigator.share(shareData);
			} catch (err) {
   		...
   		showEventTickets(event, tickets)
   		{
			...
			this.hasClickedToShowTickets = true
   		},
   		initiateTicketPurchase(ticket)
   		{
			console.log(ticket)
   			try {
                axios.post("/tickets/checkout", {ticket_id: ticket.id})
                .then(response => {
                	if( !response.data.error )
                	{
                		window.location.href =  ( this.user != null && this.user.length > 1 ) ? "/user/dashboard/ticket/checkout/".ticket.slug : "ticket/checkout/"+ticket.slug
                	}
            	})
							...
							<template x-for="day in calender_days">
								<li>
									<button type="button" @click="showTickets(day)" x-text="day.day"
										...
										:class="{'text-gray-200 border bg-red-1000': day.hasEvents, 'text-purple-1000': !day.hasEvents}"
			...
			<div class="mb-4 border-b border-gray-200 dark:border-gray-700">
			    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="default-styled-tab" data-tabs-toggle="#default-styled-tab-content" data-tabs-active-classes="text-purple-600 hover:text-purple-600 dark:text-purple-500 dark:hover:text-purple-500 border-purple-600 dark:border-purple-500" data-tabs-inactive-classes="dark:border-transparent text-gray-500 hover:text-gray-600 dark:text-gray-400 border-gray-100 hover:border-gray-300 dark:border-gray-700 dark:hover:text-gray-300" role="tablist">
			       ...
			       {{--  <li class="me-2" role="presentation">
			        ...
			        <li class="me-2" role="presentation" x-show="hasClickedToShowTickets">
	            				...
	            				<li class="border shadow-lg md:shadow-sm border-gray-200 bg-gray-200 px-2 py-5 cursor-pointer rounded text-purple-1000 hover:shadow-md">
									<div class="grid gap-2">
										<div class="flex justify-between items-center">
											...
											<svg @click="shareEvent(event)" class="text-purple-1000 w-5 font-bold cursor-pointer h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="M7.217 10.907a2.25 2.25 0 1 0 0 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186 9.566-5.314m-9.566 7.5 9.566 5.314m0 0a2.25 2.25 0 1 0 3.935 2.186 2.25 2.25 0 0 0-3.935-2.186Zm0-12.814a2.25 2.25 0 1 0 3.933-2.185 2.25 2.25 0 0 0-3.933 2.185Z" /></svg>
										</div>
										...
										<div class="py-2 grid gap-3 mt-2 text-purple-1000">
											...
											<div>
												<button @click="showEventTickets(event, event.tickets)" class="bg-purple-1000 text-gray-300 hover:bg-purple-900 hover:text-gray-100 py-3 w-full text-center hover:shadow-md">View Event ticket</button>
											</div>
										</div>
									</div>
								</li>
			       ...
			       <template x-if="chosenTickets.length > 0">
			       		<template x-for="ticket in chosenTickets">
			       			<div class="grid gap-8">
								<div class="">
									<div class="bg-white px-4 py-10 pb-5 rounded w-48 md:w-72 md:px-8 text-purple-1000 font-body mt-2 text-center border border-gray-200 shadow-md hover:shadow-lg ">
										<div>
											...
											<div class="grid grid-cols-2 gap-6 text-left mt-5">
												<div class="pb-3 border-b border-gray-300">
													...
													<a class="text-lg text-purple-1000 " x-text="user != null ? user.nickname : 'My CruiseID'">CruiseHq</a>
												</div>
												<div class="pb-3 border-b border-gray-300">
													<p class="text-greyish text-sm">Organizer</p>
													<!-- <a :href="'/promoter/' + chosenEvent.user.nickname" target="_blank"  class="text-lg text-purple-1000 cursor-pointer" x-text="chosenEvent.user.nickname">CruiseHq</a> -->
													<p class="text-lg text-purple-1000 cursor-pointer" x-text="chosenEvent.user.nickname">CruiseHq</p>
												</div>
												<div class="pb-3 border-b border-gray-300">
										...
										<div class="flex justify-end">
											<svg @click="shareTicket(ticket)" class="text-purple-1000 w-5 font-bolder cursor-pointer h-6 mt-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="M7.217 10.907a2.25 2.25 0 1 0 0 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186 9.566-5.314m-9.566 7.5 9.566 5.314m0 0a2.25 2.25 0 1 0 3.935 2.186 2.25 2.25 0 0 0-3.935-2.186Zm0-12.814a2.25 2.25 0 1 0 3.933-2.185 2.25 2.25 0 0 0-3.933 2.185Z" /></svg>
										</div>
									</div>
									...
									<div class="flex justify-center items-center mb-10 mt-4">
										<button @click="initiateTicketPurchase(ticket)" class="py-3 px-8 bg-red-1000 text-gray-100">Get ticket</button>
									</div>
								</div>
			       			</div>
			       		</template>
